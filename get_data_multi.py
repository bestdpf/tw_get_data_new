...
def get_data(idx):
	if(abit[get_mod(idx)]):
		...
		try:
			friends=t.get().friends.ids(user_id=idx);
			#f_fr=io.open(str(idx)+'/friends.json','wb')
			f_fr.write(str(friends))
			f_fr.close()
			followers=t.get().followers.ids(user_id=idx);
			#f_fl=io.open(str(idx)+'/followers.json','wb')
			f_fl.write(str(followers))
			f_fl.close()
			user_info=t.get().users.show(user_id=idx)
			#f_ui=io.open(str(idx)+'/user_info.json','wb')
			f_ui.write(str(user_info))
			f_ui.close()
			timeline=t.get().statuses.user_timeline(user_id=idx)
			#f_tl=io.open(str(idx)+'/user_timeline.json','wb')
